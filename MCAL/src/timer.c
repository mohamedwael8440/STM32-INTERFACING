...
void Timer_PWMledInit(uint8_t ledno)
{
	/* GPIOB PC (RCC, TIM1 alterate) */
		RCC->AHB1ENR 		|=  RCC_AHB1ENR_GPIOCEN;
		/* TIM3 RCC (APB1) */
			RCC->APB1ENR		|= RCC_APB1ENR_TIM3EN;

			/* TIM3 tick time - 1ms (PSC). */
...
void Timer_PWMLEDSetDuty(uint8_t ledno, uint8_t duty){
	switch (ledno) {
		case 6:
			TIM3->CCR1			 = (TIM3->ARR) * (duty/100.0);
			break;
		case 7:
			TIM3->CCR2			 = (TIM3->ARR) * (duty/100.0);
			break;
